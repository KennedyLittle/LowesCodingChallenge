package com.example.lowescodingchallenge
...
import com.example.weatherlonglat.BuildConfig
import com.example.weatherlonglat.R
...
class MainActivity : AppCompatActivity() {
    ...
    inner class WeatherTask2 : AsyncTask<String, Void, String>() {
        override fun doInBackground(vararg params: String?): String? {
            var response:String?
            try{
                response = URL("https://api.openweathermap.org/data/2.5/forecast?q={city}&appid={api key}").readText(
                    Charsets.UTF_8
                )
            }catch (e: Exception){
                response = null
